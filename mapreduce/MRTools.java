...
*/		
		...
		for(String value : values) {
			...
			String tfidfbd = "" + tfidf;
			String valueInf = idfstring[1].concat(",").concat(tfidfbd);
			wordInf.add(valueInf);
		}// end of for-loop for finding tfidf
	...
	public static String getTogetherTFIDF(List<String> wordInf) {
		...
		while(iterator.hasNext()) {
			String[] metavalue = iterator.next().split(",");
			int index = Integer.parseInt(metavalue[0])-1;
			metadata[index] = metavalue[1];
